...
# ========== ТЕМЫ С ПОДТЕМАМИ - ПОЛНОСТЬЮ РАБОЧАЯ ==========
THEORY_DETAILED = {
    "Человек и общество": {
    ...
    "Политика": {
        ...
        "Демократия и выборы": {
            ...
            "examples": [
                ...
                "Разделение властей: в России — Федеральное Собрание, Правительство, суды"
            ],
            "questions": [
        ...
        "Политические партии": {
            ...
            "examples": [
                ...
                "Идеологии: консерватизм (сохранение традиций), либерализм (свобода личности), социализм (равенство)"
            ],
            "questions": [
    ...
    "Право": {
        ...
        "Правоотношения": {
            ...
            "examples": [
                ...
                "Юридическая ответственность: штраф за нарушение ПДД, лишение свободы"
            ],
            "questions": [
    ...
    "Духовная культура": {
        ...
        "Искусство": {
            ...
            "key_points": [
                ...
                "Функции искусства: эстетическая, познавательная, воспитательная, гедонистическая",
        ...
        "Религия": {
            ...
            "key_points": [
                ...
                "Функции религии: мировоззренческая, компенсаторная, регулятивная, интегрирующая",
...
def get_theory_keyboard():
    ...
    for topic in topics:
        builder.button(text=f"📘 {topic}", callback_data=f"th_{topic}")
...
def get_subtopics_keyboard(topic):
    ...
    for subtopic in subtopics:
        builder.button(text=f"📖 {subtopic}", callback_data=f"sub_{topic}_{subtopic}")
...
def get_tasks_keyboard():
    builder = InlineKeyboardBuilder()
    for i in range(1, 25):
        builder.button(text=f"{i}", callback_data=f"tsk_{i}")
...
def get_question_keyboard(qid, qtype, exam_mode=False):
    ...
    if not exam_mode:
        builder.button(text="📚 ТЕОРИЯ", callback_data=f"th_{OGE_QUESTIONS[qid-1]['topic']}")
        builder.button(text="🎲 СЛУЧАЙНОЕ", callback_data="random_task")
        builder.adjust(1, 2)
    else:
        builder.adjust(1)
...
def get_options_keyboard(options, qid):
    ...
    builder.button(text="🔙 НАЗАД", callback_data=f"tsk_{qid}")
    builder.adjust(1)
    return builder.as_markup()
...
async def repeat_menu(m: Message):
    ...
    if weak:
        ...
        for t in weak:
            ...
            kb.button(text=f"📘 {t['topic']}", callback_data=f"th_{t['topic']}")
...
@dp.callback_query(lambda c: c.data.startswith("th_"))
async def cb_theory_topic(c: CallbackQuery):
    topic = c.data.replace("th_", "")
...
async def cb_subtopic(c: CallbackQuery):
    ...
    if len(parts) == 3:
        ...
        if topic in THEORY_DETAILED and subtopic in THEORY_DETAILED[topic]:
            ...
            kb = InlineKeyboardBuilder()
            kb.button(text="◀️ НАЗАД К ПОДТЕМАМ", callback_data=f"th_{topic}")
            await c.message.edit_text(text, reply_markup=kb.as_markup())
    await c.answer()

@dp.callback_query(lambda c: c.data.startswith("tsk_"))
async def cb_task(c: CallbackQuery):
    try:
        num = int(c.data.replace("tsk_", ""))
        if 1 <= num <= 24:
...
async def handle_text(m: Message):
    ...
    if user.waiting_for_answer and user.current_question:
        ...
        if ans:
            ...
            # Если это вариант ОГЭ и есть следующие задания
            if exam_mode and user.current_exam:
                if user.current_exam_index < len(user.current_exam) - 1:
                    pass
                else:
                    total = sum(q["points"] for q in user.current_exam)
...
async def main():
    ...
    print(f"📚 Теория: 6 тем, 30+ подтем - ПОЛНОСТЬЮ РАБОТАЕТ")
    ...
    print(f"✅ Проверка ответов: СТРОГАЯ (баллы только за правильные)")
