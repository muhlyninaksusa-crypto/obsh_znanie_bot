...
def get_tasks_group_keyboard():
    ...
    for text, callback in groups[:3]:
        builder.button(text=text, callback_data=callback)
    
    builder.button(text=groups[3][0], callback_data=groups[3][1])
    builder.button(text=groups[4][0], callback_data=groups[4][1])
...
async def full_exam_command(message: types.Message):
    ...
    # Создаем полный вариант из 30 вопросов
    exam_questions = []
    for i in range(1, 31):
        task_key = f"task_{i}"
    ...
    await message.answer(
        ...
        "Вам предстоит 30 заданий в правильном порядке как в реальном экзамене.\n"
...
async def send_question(user_id, question, edit_message=None, is_exam=False):
    ...
    if is_exam:
        exam_progress = f"<b>📝 ВАРИАНТ ОГЭ | Задание {task_num}/30</b>\n\n"
    else:
...
async def main():
    ...
    print("📚 База вопросов: 30 заданий ОГЭ")
